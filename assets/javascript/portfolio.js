...
Credits:
    - Lummit - https://portfolio.hardik.live/ | https://github.com/Lumm1t/ | Discord: Lummit#0201
...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="https://bit.do/${link.link}" target="_BLANK">${link.name}</a>`);
  ...
  app.titleChanger(['LiquidBounce++', 'Lantern', '', 'OpenSource', 'Free', 'Forge Mod', 'Ghost Client', 'lbplusplus.ga']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>portfolio.hardik.live $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
